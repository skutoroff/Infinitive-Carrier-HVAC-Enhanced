...
However, after days of using the USB interface, a new problem(s) were found. First, the cron 4 process which purges old log files does not allow the running process to open new log files. So, after the log files are purged, the program does a forced os.Exit and lets Systemd sort out the issue.
Next, twice the Infinitive.csv file was not being rotated and the chart was not being created after the first day. So, a forced os.Exit was added cron 3 to let Systemd sort this out until the cause is found.
