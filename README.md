...
To use the executable Pi file, install it in /var/lib/infinitive/ and set it up in systemd so the crash restarts don't get irritating. They occur about hourly on average. The current chart reports the average number of restarts per day as the MTBR (mean time between restarts). I've seen a high of about 30 per day and a low of about 18. If you set systemd to save output and error log files as I did, save them in /var/log/infinitive/ as in my provided infinitive.service file and they will be deleted 3x per month to manage their size. I have not tried the orginal authors suggestion to use the USB interface and set USB 1.1 mode, too radical.
